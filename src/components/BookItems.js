                        ...
                        <div className="card-body">
                            {Object.keys(verses || {}).map(keys => (
                                <div className='p-1 py-2 bg-dark bg-opacity-10 border border-info border-top-0 border-start-0 border-end-0' key={keys}>
                                    <span className="badge bg-secondary mb-0  bg-opacity-25 py-2 px-1 rounded-5 text-white-50 fw-semibold ms-2 float-end" style={{ fontSize: '12px' }}>{verses[keys].id}</span><p className="card-text text-end  lead" style={{ fontSize: '1.2rem' }}>&#9900; {verses[keys].content}</p>
